...
public class OrderDao extends AbstractJdbcDao<Order, Integer> implements GenericDao<Order, Integer> , OrderExpandedDao {
    ...
    private static final String INSERT_NEW_ORDER_QUERY =
            "INSERT INTO delivery_order ( id_customer , id_courier , price, id_status , start_point ,finish_point , description , start_time , finish_time , expected_time, id_cargo_type , id_transport_type ) " +
                    "VALUES ( ? , ? , ? , ? , ? , ? , ? , ?, ? , ? , ? , ?)";
    private static final String UPDATE_ORDER_QUERY = "UPDATE delivery_order SET " +
            "id_customer = ? , id_courier = ? , price = ? , id_status = ? , start_point = ? , finish_point = ? , description = ? , start_time = ? , finish_time = ?, expected_time = ? , id_cargo_type = ? , id_transport_type = ?" +
            "WHERE id = ?";
